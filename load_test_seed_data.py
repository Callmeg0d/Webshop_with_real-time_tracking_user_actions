...
async def main() -> None:
    """Основная функция для загрузки всех seed данных в ТЕСТОВУЮ БД."""
    print("=" * 60)
    print("[START] Начало загрузки seed данных в ТЕСТОВУЮ БД")
    print(f"[INFO] Используется БД: {settings.TEST_DATABASE_URL}")
    print("=" * 60)
    
    # Создаём engine и session для ТЕСТОВОЙ БД
